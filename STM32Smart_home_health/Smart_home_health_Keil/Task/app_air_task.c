...
void air_task(void *pvParameters)
{
    ...
    {
		air_time++;
        if (8 < air_time && NULL != xSensorDataQueue)
        {
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
    ...
    {
        data.TVOC = ((float)rx_buffer_air[2] * 256 + (float)rx_buffer_air[3]) * 0.01;
        data.HCHO = ((float)rx_buffer_air[4] * 256 + (float)rx_buffer_air[5]) * 0.01;
        data.CO2 = (float)rx_buffer_air[6] * 256 + (float)rx_buffer_air[7];
        Airquality_rev_data((uint8_t *)rx_buffer_air, sizeof(rx_buffer_air)); // 再次启动接收
    }
    if (huart->Instance == USART6)
    {
        data.PM25 = (int)rx_buffer_pm25[1] * 128 + (int)rx_buffer_pm25[2];
